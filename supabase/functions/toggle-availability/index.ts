...
Deno.serve(async (req) => {
  ...
  try {
    const { creator_id, status } = await req.json();
    ...
    // Kalau jadi tersedia, cek antrian
    if (status === 'tersedia') {
...
async function processQueue(creatorId: string) {
  // Ambil data creator untuk cek spesialisasi
  const { data: creator } = await supabase
    .from('creators')
    .select('id, name, wa_number, specializations, active_tasks')
    .eq('id', creatorId)
    .single();

  if (!creator) return;

  // Cari antrian yang cocok dengan spesialisasi creator (urutan terlama dulu)
  ...
  // Blast WA ke advertiser — creator sudah ditemukan
